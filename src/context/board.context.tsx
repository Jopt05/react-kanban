...
export const BoardProvider = ({children}: any) => {
    ...
    const updateBoard = async(id: string, name: string) => {
        try {
            const response = await kanbanApi.put(`/boards/${id}`, { name });
            boardDispatch({
                type: 'setBoardsList',
    ...
    const deleteBoard = async(id: string) => {
        showLoader()
        try {
            await kanbanApi.delete(`/boards/${id}`);
            boardDispatch({
                type: 'setBoardsList',
                payload: boardState?.boardsList?.filter(board => board.id !== id)
            })
            if( boardState.boardsList?.length > 0 ) {
                boardDispatch({
                    type: 'setSelectedBoard',
                    payload: boardState?.boardsList?.[0]
                })
            }
        } catch (error) {
            console.log(error)
